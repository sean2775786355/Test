                ...
                <div class="row">
                  <div class="col-md-4">
                    <div class="form-group">
                      <label>物料名称</label>
                      <%int i=0; %>
                      <select id="materiel_id" name="materiel_id" onchange="change();" class="form-control select2">
                        <option> </option>
                        <c:forEach items="${materielList}" var="materielList">
                        <option value="${materielList.id}"  id="<%=++i %>">${materielList.name}</option>
                        </c:forEach>
                      </select>
                    </div>  
                  </div>
...
物料编号：${materielList.code}
供应商名称：${materielList.productor_name}
物料类型：${materielList.type}
标准号：${materielList.standard_code}
规格：${materielList.standard}
级别：${materielList.level}
库存：${materielList.storage}
计量单位：${materielList.unit}
单价：${materielList.cost} 
                      </textarea>
                      </c:forEach>
  ...
  $(function () {
    ...
    $('#equipment_info').DataTable({
  	...
  	function change(){
		...
		var option = $("#materiel_id option:selected");
    ...
    function  submit(data) {
        ...
        $.ajax({
            url: root+'/materiel/materielApplyPost',
            type: 'POST',
            dataType: 'json',
            data: data,
        }).done(function (data) {
